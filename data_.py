# -*- coding: utf-8 -*-

import os, file_
from io import BytesIO
from itertools import cycle


class DataManager():

    def __init__(self, client_path: str) -> None:

        self.success = 0
        self.errors = 0

        self.pack_path = os.path.join(
            client_path,
            "files",
            "data.pack"
        )
        self.pck_path = os.path.join(
            client_path,
            "files",
            "data.pck"
        )
        self.data_path = os.path.join(
            client_path,
            "files",
            "data",
            ""
        )

    # DECRYPTOR
    def xor(self, data: bytearray, key: bytearray) -> bytes:

        return bytes(a ^ b for a, b in zip(data, cycle(key)))
    
    def start_decrypting(self) -> bool:

        try:

            key = b''
            stream = BytesIO(open(self.pack_path, "rb").read())

            with open(self.pck_path, "wb") as pck:

                while True:

                    decrypted_bytes = self.xor(stream.read(len(key)), key)
                    pck.write(decrypted_bytes)

                    if decrypted_bytes == b'':
                        break
                    
                return True
            
        except:
            return False

    # EXTRACTOR
    def read_header(self, header: bytes) -> int:

        if header[4:5] == b'\x02' and header[10:] == b'\x00':

            length_block = int.from_bytes(
                header[:4],
                byteorder='little',
                signed=False
            )

            length_path = int.from_bytes(
                header[5:6],
                byteorder='little',
                signed=False
            )

            length_container = int.from_bytes(
                header[6:10],
                byteorder='little',
                signed=False
            )

            return length_block, length_path, length_container
        
        else:
            return 0, 0, 0

    def start_extracting(self) -> bool:

        pck_file = file_.open_file(
            file_path = self.pck_path,
            write=False
        )

        mfile = file_.open_memory(
            file = pck_file,
            length = os.path.getsize(os.path.join(self.pck_path))
        )

        cur = 0
        flag = 0

        while True:

            if cur >= flag:
                flag = cur

                try:

                    cur = mfile.find(b'\x02', cur)
                    mfile.seek(cur - 4)

                    length_block, length_path, length_container = self.read_header(mfile.read(11))

                    if length_path or length_container != 0:

                        if length_block == length_path + length_container + 19:

                            cur = mfile.tell()
                            mfile.seek(cur + 4)

                            path = mfile.read(length_path).decode("utf-8", "ignore").replace("/", "\\")
                            full_path = os.path.join(self.data_path, path)

                            container = mfile.read(length_container)

                            status = file_.write_file(path=full_path, container=container)
                            if status:
                                print("EXTRACTED: ", path)
                            else:
                                print("SKIPED: ", path)

                            cur += 4 + length_path + length_container + 4
                            self.success += 1
                            continue

                except:
                    self.errors += 1

                cur += 1
                continue

            else:
                break
        
        return True